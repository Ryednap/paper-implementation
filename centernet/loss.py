...
def _neg_loss(preds: List[torch.Tensor], target: torch.Tensor) -> torch.Tensor:
    """
    ...
    return cast(torch.Tensor, loss / len(preds))
...
def _embedding_loss(
    ...
    return cast(torch.Tensor, pull_loss / len(tl_embd_list)), cast(
        torch.Tensor, push_loss / len(tl_embd_list)
    )
...
def _regs_loss(
    ...
    return cast(torch.Tensor, loss / len(regs))


def centernet_loss(
    batch: Dict[str, torch.Tensor], output: List[Dict[str, torch.Tensor]]
    ...
    ct_hmap = [x["ct_hmap"] for x in output]

    tl_embd = [
        transpose_and_gather_feat(x["tl_embd"], ind=batch["tl_inds"]) for x in output
    ]
    br_embd = [
        transpose_and_gather_feat(x["br_embd"], ind=batch["br_inds"]) for x in output
    ]

    tl_regs = [
        transpose_and_gather_feat(x["tl_regs"], ind=batch["tl_inds"]) for x in output
    ]
    br_regs = [
        transpose_and_gather_feat(x["br_regs"], ind=batch["br_inds"]) for x in output
    ]
    ct_regs = [
        transpose_and_gather_feat(x["ct_regs"], ind=batch["ct_inds"]) for x in output
    ]

    focal_loss = (
        _neg_loss(tl_hmap, batch["tl_hmap"])
        + _neg_loss(br_hmap, batch["br_hmap"])
        + _neg_loss(ct_hmap, batch["ct_hmap"])
    )
    reg_loss = (
        _regs_loss(tl_regs, batch["tl_regs"], valid_mask=batch["ind_masks"])
        + _regs_loss(br_regs, batch["br_regs"], valid_mask=batch["ind_masks"])
        + _regs_loss(ct_regs, batch["ct_regs"], valid_mask=batch["ind_masks"])
    )
